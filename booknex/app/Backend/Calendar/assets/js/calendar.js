...
function reloadCalendarFn()
{
		...
		function(result )
	{
		...
		FSCalendarViewType = FSCalendar.view.type;
	...
	$(document).ready(function()
	{
		...
		FSCalendar = new FullCalendar.Calendar( $("#fs-calendar")[0],
		{
			...
			datesRender: function()
			{
				...
				if(FSCalendarViewType !== FSCalendar.view.type) reloadCalendarFn();
			...
			eventRender: function(info)
			{
				...
				if( data.customers_count == 1 && data.status )
				{
					...
					html += '<div class="calendar_event_line_3 d-flex align-items-center" style="gap: 5px">' + booknetic.htmlspecialchars( data.customer ) + ' <span class="appointment-status-default d-flex justify-content-center" style="background-color: ' + data.status.color + '"><i class="' + data.status.icon + '"></i></span></div>';
				}
