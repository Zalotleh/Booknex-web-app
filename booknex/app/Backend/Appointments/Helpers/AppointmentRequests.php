...
class AppointmentRequests
{
    ...
    {
        $keyExists = array_key_exists($this->current, $this->appointments);

        if ($keyExists)
            return $this->appointments[$this->current];

        return end($this->appointments);
    }
    ...
    public function getPrices( $sumForAllRecurringAppointments = false ): array
    {
        ...
        foreach ($this->appointments as $key => $appointment)
        {
			...
			$prices = $newSumForAllRecurringAppointments ? $appointment->getGroupedPrices() : $appointment->getPrices();
